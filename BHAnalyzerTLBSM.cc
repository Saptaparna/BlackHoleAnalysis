...
class BHAnalyzerTLBSM : public edm::EDAnalyzer {
  ...
  bool firedHLT_PFHT600_v1;
  //bool firedHLT_PFHT650_v2;
  bool firedHLT_PFHT900_v1;
...
BHAnalyzerTLBSM::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  const edm::View<pat::Jet> & jets = *jetHandle;
   ...
   firedHLT_PFHT600_v1  = false;
   //firedHLT_PFHT600_v2  = false;
   //firedHLT_PFHT650_v2  = false;
   firedHLT_PFHT900_v1  = false;
   //TODO
   ...
   */ 
   ...
   if (h_trigRes->size()!=triggerList.size()) std::cout << "ERROR: length of names and paths not the same: " 
                                                << triggerList.size() << "," << tr.size() << endl;
   // dump trigger list at first event
   cout << "h_trigRes->size() = " << h_trigRes->size() << endl;
   cout << "triggerList.size() = " << triggerList.size()  << endl; 
   for (unsigned int i=0; i<triggerList.size(); i++) {
     std::cout << "["<<i<<"] = " << triggerList.at(i) << endl; //<<setw(40)<<endl;
     //std::cout << " Prescale " << triggerPrescales->getPrescaleForIndex(i) << ": " << (h_trigRes->accept() ? "Event Passed" : "Event Failed") << endl;
     if ( !h_trigRes->accept() == 1 ) continue;
     ...
     if( triggerList[i] == "HLT_PFHT600_v1")  { firedHLT_PFHT600_v1  = true; }
     //if( triggerList[i] == "HLT_PFHT650_v2")  { firedHLT_PFHT650_v2  = true; }
     if( triggerList[i] == "HLT_PFHT900_v1")  { firedHLT_PFHT900_v1  = true; }

   }
   //cout << "here line 573" << endl;
  /* 
...
void BHAnalyzerTLBSM::beginJob()
{
  ...
  tree->Branch("firedHLT_PFHT600_v1",&firedHLT_PFHT600_v1,"firedHLT_PFHT600_v1/B");
  //tree->Branch("firedHLT_PFHT650_v2",&firedHLT_PFHT650_v2,"firedHLT_PFHT650_v2/B");
  tree->Branch("firedHLT_PFHT900_v1",&firedHLT_PFHT900_v1,"firedHLT_PFHT900_v1/B");
